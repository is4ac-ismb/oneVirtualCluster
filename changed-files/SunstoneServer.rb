...
require 'OpenNebulaJSON/VirtualClusterJSON'
...
class SunstoneServer < CloudServer
    ...
    def get_pool(kind,gid)
        ...
        pool = case kind
			...
			when "vcluster"	  then VirtualClusterJSON.get_pool(Group.build_xml, @client)	#Antonio
...
#File.open("/var/log/one/out.txt", 'a') {|f| f.write("\n\n PERFORM ACTION CALLED:\n" + kind + id + "\n:end\n\n") }
		if kind == "vcluster"
			resource = 	VirtualClusterJSON.get_vcluster(id, Group.build_xml, @client)
			return [200, resource]
		end
#Antonio - end adds
    ...
    def create_resource(kind, template)
        resource = case kind
			...
			when "vcluster"		then VirtualClusterJSON.new(Group.build_xml, @client) #Antonio
...
#Antonio - start adds
		if kind == "vcluster"
			resource = 	VirtualClusterJSON.delete_vcluster(id, Group.build_xml, @client)
			return [204, resource]
		end
#Antonio - end adds
...
#Antonio - start adds
		if kind == "vcluster"
			resource = 	VirtualClusterJSON.perform_action(id, action_json, Group.build_xml, @client)
			return [204, resource]
		end
#Antonio - end adds
